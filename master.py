...
if 1==1: #slack_client.rtm_connect():
...
class Morbidostat:
    ...
    def start(self):
        ...
        # self.history = self.slack_client.api_call("channel.history", channel=self.chanid, count = 1)
        ...
        self.bars_drug_interval = self.config[self.varstr].getfloat(('bars_drug_pump_interval')) #In seconds (for 'bars' alg)
    ...
    def control_alg(self):
        try:
            ...
            if self.selection == 'bars': #NEW
                ...
                if (self.elapsed_time.total_seconds() > (self.last_dilutiontime + int(self.bars_drug_interval))):  #both were converted into seconds
                    self.pump_drug()
                    self.pump_waste()
                    self.pump_drug()
                    self.pump_waste()
